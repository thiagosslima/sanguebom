...
class NotificationResourceTest {
    ...
    void shouldReturnAllNotifications() throws Exception {
        given(notificationRepository.findAll(any(Sort.class)))
                .willReturn(List.of(notification(USER_ID, NotificationStatus.SENT), notification(2L, NotificationStatus.PENDING)));
    ...
    void shouldFilterByUserId() throws Exception {
        given(notificationRepository.findAll(any(Sort.class)))
                .willReturn(List.of(notification(USER_ID, NotificationStatus.SENT), notification(2L, NotificationStatus.SENT)));
    ...
    @Test
    @DisplayName("POST /api/notifications com type fora do NotificationType devolve 400")
    ...
    @Test
    @DisplayName("GET /api/notifications?status= fora do NotificationStatus devolve 400 como ProblemDetail")
    void shouldRejectInvalidStatusFilter() throws Exception {
        mockMvc.perform(get("/api/notifications").param("status", "NAO_EXISTE"))
                ...
                .andExpect(jsonPath("$.['parâmetros inválidos'][0].field").value("status"));
